...
class Controlador:
    """
    Clase que gestionara los eventos y cambios a la ventana, cantidad de juegos actuales
    y la manipulacion de los orovenientes de las clases para scraping.
    """
    ...
    def agregar(self, entrada):
        ...
        if datos_juego == 0:
            ...
            logo_juego = im.cargarImagen(
                im.descargarImagen(
                    datos_juego[2], entrada.get().replace(" ", "_"))
            )
            ...
            self.__ventana.sumar_a_total(
                self.__lista_juegos[-1].getPrecioFinal())
